...
| Secret | Wo | Zweck |
|--------|-----|--------|
| **`MASTER_KEY`** | nur Env | Verschlüsselt Secrets **in der App-DB** (Client Secret, Store-Passwort) |
| **Tenant-Store-Passwort** | DB (encrypted) + Offline-Recovery-Export | Öffnet Blobs/Manifeste dieses Tenants (`store_password`) |
...
- **Disaster Recovery ohne diese App:** Wenn die VM tot ist, brauchst du Store-Root + Store-Passwort
  und `m365-restore`. MASTER_KEY und DB sind dann egal.
- **MASTER_KEY kompromittiert ≠ Stores offen:** Angreifer mit nur dem Master-Key und der DB kann
  die Store-Passwörter entschlüsseln — deshalb MASTER_KEY schützen. Aber wer nur das Store-Passwort
...
Beim Tenant-Create: `RandomPassword(32)` → verschlüsseln → als `store_password` speichern →
Store-Verzeichnisse anlegen.
...
- Azure Secrets, Zertifikate, Store-Passwörter
